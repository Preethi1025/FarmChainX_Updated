...
public class BatchService {
    ...
    public BatchRecord approveBatch(String batchId, String distributorId) {
        ...
        for (Crop crop : crops) {
            ...
            // Quantity from crop
            listing.setQuantity(
                    crop.getQuantity() != null ? crop.getQuantity() : 0.0
            );
    ...
    public BatchRecord splitBatch(String parentBatchId, double splitQuantity, String userId) {
        ...
        for (Crop pc : parentCrops) {
            ...
            if (childQty > 0) {
                ...
                childCrop.setQuantity(childQty);
    ...
    private Map<String, Object> toBatchResponse(BatchRecord batch) {
        ...
        if (listing != null) {
            ...
            data.put("availableQuantity",
                    cropRepository.getAvailableQuantityByBatch(batch.getBatchId()));
            data.put("listingStatus", listing.getStatus());
        } else {
            data.put("price", 0);
            data.put("availableQuantity", 0);
