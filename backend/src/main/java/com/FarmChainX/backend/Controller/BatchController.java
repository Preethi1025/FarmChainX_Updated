...
public class BatchController {
        ...
        public ResponseEntity<List<BatchRecord>> getBatchesByFarmer(
        ...
        public ResponseEntity<List<Crop>> getCropsForBatch(
                        @PathVariable String batchId) {

                return ResponseEntity.ok(
                                cropRepository.findByBatchId(batchId));
        }
        ...
        public ResponseEntity<List<Map<String, Object>>> getApprovedBatches(
        ...
        public ResponseEntity<?> approveBatch(
                ...
                return ResponseEntity.ok(
                                batchService.approveBatch(batchId, distributorId));
        }
        ...
        public ResponseEntity<BatchRecord> updateBatchStatus(
                ...
                if ("QUALITY_UPDATED".equalsIgnoreCase(status)) {
                        ...
                        String grade = body.get("qualityGrade") != null
                                        ? body.get("qualityGrade").toString()
                                        : null;

                        Integer confidence = null;
                        if (body.get("confidence") != null) {
                                try {
                                        confidence = Integer.valueOf(
                                                        body.get("confidence").toString());
                                } catch (NumberFormatException ignored) {
                                }
                        }

                        return ResponseEntity.ok(
                                        batchService.updateQualityGrade(
                                                        batchId, grade, confidence, userId));
                }

                return ResponseEntity.ok(
                                batchService.updateStatus(batchId, status, userId));
        }
        ...
        public ResponseEntity<BatchRecord> splitBatch(
                ...
                return ResponseEntity.ok(
                                batchService.splitBatch(batchId, quantity, userId));
        }
        ...
        public ResponseEntity<List<BatchRecord>> mergeBatch(
                ...
                return ResponseEntity.ok(
                                batchService.mergeBatches(
                                                targetBatchId, sourceBatchIds, userId));
        }

        // ------------------- TRACE -------------------
        @GetMapping("/{batchId}/trace")
        public ResponseEntity<Map<String, Object>> getBatchTrace(
                        @PathVariable String batchId) {
